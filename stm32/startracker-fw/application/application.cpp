...
void uart_isr() { serial_rpi.ISR(&huart1); }

void vcp_receive_cb(uint8_t* buf, uint32_t len) { serial_usb.vcp_receive_cb(buf, len); }

void vcp_send_cmplt_cb() { serial_usb.vcp_send_cmplt_cb(); }
...
static void task_ui() {
    ...
    while (1) {
        ...
        // only update IMU when I2C is ready. tha means the DMA transfer(s) from the display are
        // finished.
...
static void task_motor() {
    ...
    while (1) {
        ...
        if (event & EVENT_TASK_MOTOR_TIMER) {
            perf.motor.start();
            motor_update();  // update all the steppers and related functions. May block for longer
                             // time if required.
            perf.motor.end();
        }
    }
