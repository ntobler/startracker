...
// static float polynom[3][N] = {{ 7.11859163e-12f,  1.78833188e-07f,  3.31593291e-03f,
// -1.45327218e-09f },
// { 3.55347520e-11f,  5.26166407e-07f,  9.16399197e-03f, -6.48713382e-09f },
//						      { 1.27991415e-11f,  1.70074972e-07f,  6.23996244e-03f,
//-1.54755701e-09f }};
    ...
    switch (motor.state) {
        ...
        case MOTOR_MODE_READY:
            if (motor.flags & MOTOR_DO_HOME) {
                ...
                motor.state = MOTOR_MODE_HOMING;
                home_timer = 0;
            } else if (motor.flags & MOTOR_DO_START && motor.trajectory_ready) {
                reset_command_flag(MOTOR_DO_START);
                motor.state = MOTOR_MODE_RUNNING;
            } else if (motor.flags & MOTOR_DO_DISABLE) {
                reset_command_flag(MOTOR_DO_DISABLE);
                motor.state = MOTOR_MODE_NONE;
            } else if (motor.flags & MOTOR_DO_ASSIGN_TRAJECTORY) {
        ...
        case MOTOR_MODE_RUNNING:
            if (motor.flags & MOTOR_DO_STOP) {
                ...
                motor.state = MOTOR_MODE_READY;
            } else if (motor.flags & MOTOR_DO_PAUSE) {
        ...
        case MOTOR_MODE_PAUSE:
            if (motor.flags & MOTOR_DO_RESUME) {
                ...
                motor.state = MOTOR_MODE_RUNNING;
            } else if (motor.flags & MOTOR_DO_STOP) {
                reset_command_flag(MOTOR_DO_STOP);
                motor.state = MOTOR_MODE_READY;
            }
            break;
...
void motor_do(Motor_command_flags_t cmd) { set_command_flag(cmd); }
...
static void enable_disable_motors(uint32_t enable) {
    ...
    if (old != enable) {
        ...
        if (enable) {
            ...
            // enable 12V power boosting. The 4.8V may not be enough for the drivers to start
            // properly. there is an issue that enabling the 12V booster can stress the 5.2V power
            // booster and the battery management so much that it shuts down because of overcurrent.
            //   =>  Simulations have shown that the 12V booster needs about ~100us to load all
            //   secondary capacitors,
            //       which is a considerable amount of charge. The solution is to enable it multiple
            //       times for 5us at one time with 1ms pauses in between.
            scheduler_task_sleep(1);
            for (int i = 0; i < 200; i++) {
                HAL_GPIO_WritePin(MOTOR_BOOST_ENABLE_GPIO_Port, MOTOR_BOOST_ENABLE_Pin,
                                  GPIO_PIN_SET);
            ...
            // send TMC init sequence. Since the data line is shared this needs only be done on one
            // motor.
...
static int32_t get_start_tick() { return start_time / TICK_TIME; }

static int32_t get_end_tick() { return end_time / TICK_TIME; }
