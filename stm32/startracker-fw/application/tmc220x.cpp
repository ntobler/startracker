...
void TMC220X::send_init() {
    ...
    send_data(REG_GCONF,
              ...
              gconf_internal_Rsense | gconf_en_SpreadCycle |
                  //			gconf_shaft_reverse |
                  //			gconf_index_otpw |
                  //			gconf_index_step |
 ...
 */
void TMC220X::set_to(int32_t pos) { _pos = pos; }
...
static uint32_t byteswap(uint32_t data) {
    ...
    return ((data >> 24) & 0xFF) | ((data >> 8) & 0xFF00) | ((data << 8) & 0xFF0000) |
           ((data << 24) & 0xFF000000);
}
