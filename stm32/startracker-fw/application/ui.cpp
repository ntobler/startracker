...
static Image_description_t* img_batterys[] = {
    &img_battery0, &img_battery1, &img_battery2, &img_battery3, &img_battery4,  &img_battery5,
    &img_battery6, &img_battery7, &img_battery8, &img_battery9, &img_battery10,
};
static const char* entrys[MENU_COUNT] = {
    "rpi",          "tracking",    "trajectory0", "trajectory1", "trajectory2",
    "bubble level", "view camera", "shutdown",    "about",
};
...
AbstractUI* Home::update(Ui_event_en e) {
    ...
    if (button_up.event == BTN_RISING || button_left.event == BTN_RISING ||
        button_right.event == BTN_RISING || button_down.event == BTN_RISING) {
        return &menu;
    }
...
AbstractUI* Menu::update(Ui_event_en e) {
    ...
    if (button_up.event == BTN_LONG || button_right.event == BTN_RISING) {
        ...
        if (pos == MENU_TRAJECTORY0) {
            if (motor.state == MOTOR_MODE_READY || motor.state == MOTOR_MODE_RUNNING ||
                motor.state == MOTOR_MODE_PAUSE) {
                motor_do(MOTOR_DO_DISABLE);
            }
        }
        if (pos == MENU_TRAJECTORY1) {
            if (motor.state == MOTOR_MODE_NONE || motor.state == MOTOR_MODE_READY) {
        ...
        if (pos == MENU_TRAJECTORY2) {
            if (motor.state == MOTOR_MODE_NONE || motor.state == MOTOR_MODE_HOMING ||
...
void LowBattery::draw() {
    ssd1306_Clear();
    ssd1306_DrawImage(128 / 2 - img_low_battery.width / 2, 64 / 2 - img_low_battery.height / 2,
                      &img_low_battery);
    ssd1306_UpdateScreen();
}
