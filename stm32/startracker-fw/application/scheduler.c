...
__attribute((naked)) __attribute__((optimize("O0"))) void scheduler_pendSV_handler() {
    ...
    if (currentTask) {
        ...
        asm volatile("mov r3, r8  \n push {r3}"
                     :
                     :
                     : "r3", "memory");  // these registers can not be handled by push
        asm volatile("mov r3, r9  \n push {r3}"
                     :
                     :
                     : "r3", "memory");  //"r3" in the clobber list informs the compiler that r3
                                         // will be used in this section
        asm volatile("mov r3, r10 \n push {r3}"
                     :
                     :
                     : "r3", "memory");  //"memory" in the clobber list informs the compiler that
                                         // memory content may have changed
        asm volatile("mov r3, r11 \n push {r3}"
                     :
                     :
                     : "r3", "memory");  //"sp" in clobber list is deprecated and not needed
