...
void control_update() {
    ...
    //	control.charger_charging = HAL_GPIO_ReadPin(CHARGER_CHARGING_GPIO_Port,
    // CHARGER_CHARGING_Pin) == GPIO_PIN_RESET;  //this does not work because pullop R10 is missing
    // and power_enable is pulling the line low anyhow.
    ...
    //	control.charger_done = HAL_GPIO_ReadPin(CHARGER_DONE_GPIO_Port, CHARGER_DONE_Pin) ==
    // GPIO_PIN_RESET;
    ...
    control.charge_level =
        battery_charge_level_state_machine(control.charge_level, control.battery_voltage * 1000.0f);
    ...
    switch (control.state) {
        ...
        case CONTROL_RPI_BOOTING:
            ...
            } else if (control.flags & CONTROL_RPI_SHUTDOWN) {
...
void control_do(Control_flags_t cmd) { set_command_flag(cmd); }
...
static void measure_vbat() {
    ...
    //		//float voltage = __LL_ADC_CALC_DATA_TO_VOLTAGE(vrefint, adc_bat,
    // LL_ADC_RESOLUTION_12B) / 1000.0f * 2.0f; 		float vbat = (vrefint / 1000.0f) *
    // (adc_bat / 4096.0f) * 2.0f; 		if (vbat < 6.0f && vbat > 2.0f) {
    // sum += vbat; 			count++;
    //		}
    //	}
